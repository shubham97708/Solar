...
function all(res) {
    pool.query(`select id, name, modelno , gadget , discount ,(select name from brand where brand.id = ${table}.brandid) as brand from ${table}`, (err, result) => {
        if (err) throw err;
        else {
...
router.post('/search', (req, res) => {
    ...
    pool.query(`select id, name, modelno , gadget , discount ,(select name from brand where brand.id = ${table}.brandid) as brand from ${table} where name LIKE '%${req.body.fname}%' `, (err, result) => {
        if (err) throw err;
        else {
            res.render('model/all', {
                data: result
...
router.get('/edit/:id', (req, res) => {
    ...
    pool.query(`select id, name, brandid, modelno,discount,gadget, (select name from brand where brand.id = ${table}.brandid) as brand from ${table} where id = ?`, id, (err, result) => {
        if (err) throw err;
        else {
