...
router.post('/bulk', (req, res) => {
    ...
    console.log("bulk req.body feedstock ", req.body);
    let data = [];

    for (let i = 1; i <= parseInt(totalrows); i++) {
        data[i - 1] = [modelid, brandid, vendorid, req.body[`imeino${i}`], req.body[`price${i}`], req.body[`color${i}`]];
    }
    console.log("bulk data feedstock ", data);
    const query = `insert into feedstock(modelid, brandid, vendorid,imeino, price, color) values ?`
